...
const projects: Project[] = [
  ...
  {
    id: 4,
    title: 'Regulatory Operations Table System',
    shortDesc: "An async data table demo for operational compliance records, showing debounced search, server-style filtering, pagination, virtualization, caching, optimistic edits, rollback, and error recovery.",
    longDesc: "This demo shows how a complex frontend interface handles real-world data interaction patterns.\n\nThe application presents a regulatory operations table with debounced search, server-style filtering, pagination, virtualization, caching, optimistic edits, rollback, and error recovery. It is designed as a focused frontend systems demo rather than a full business application.\n\nThe interface uses a distinctive dark, fantasy-inspired visual style, but the content and workflows remain grounded in realistic operational compliance data. Records include institutions, report types, jurisdictions, deadlines, validation states, risk levels, assigned reviewers, error counts, and submission statuses.\n\nA key part of the project is the separation between UI state and async server-like state. The table supports responsive user interactions while simulated API behavior introduces latency, failure states, cached results, and mutation recovery. This makes the demo useful for showing how a frontend can remain stable and predictable even when data is delayed, updated, or temporarily unavailable.\n\nOne implementation challenge involved the expandable architecture footer. In Chromium-based browsers, a focusable truncated text element could lose its visible ellipsis on focus, even though the standard `overflow`, `text-overflow`, and `white-space` rules were correctly applied. I solved this by preventing text-selection behavior from interfering with the collapsed state while keeping the footer keyboard-accessible. The tradeoff was intentional: the footer text is not selectable, but the visual behavior remains reliable.\n\nOpsGrid demonstrates my interest in building data-heavy React interfaces that are clear, resilient, and polished. It focuses on the kind of frontend engineering often needed in internal tools, regulatory software, operational dashboards, and expert-user applications.",
    thumbnail: '/assets/OpsGrid.png',
    fullImage: '/assets/OpsGrid.png',
    demoUrl: 'https://szlaci3-opsgrid.netlify.app/dist',
    repoUrl: 'https://github.com/szlaci3/opsgrid',
    technologies: [
      "React",
      "TypeScript",
      "Vite",
